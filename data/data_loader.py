...
def load_csv(filepath):
    ...
    # Convert text columns to lowercase - FIXED
    for column_name in df.columns:
        # Create new list with lowercase values
        lowercase_values = []
        for value in df[column_name]:
            if pd.isna(value):
                lowercase_values.append('null')
            elif isinstance(value, str):
                lowercase_values.append(value.lower())
            else:
                lowercase_values.append(value)
        df[column_name] = lowercase_values
